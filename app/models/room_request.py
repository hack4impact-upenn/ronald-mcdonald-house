        ...
        def generate_fake(count=5, **kwargs):
            ...
            for i in range(count):
                rr = RoomRequest(
                        ...
                        relationship_to_patient = random.choice(["Father", "Mother", "Parent"]),
                        ...
                        email = fake.email(),
                        primary_language = random.choice(["English", "Spanish""]),
                        secondary_language = random.choice(["English", "Spanish", "Japanese", "ASL"]),
                        previous_stay = fake.boolean(),
                        patient_full_name = fake.name(),
                        patient_dob = fake.past_date(),
                        patient_gender = random.choice(["Male", "Female", "Non Binary"]),
                        patient_hospital = random.choice(["Children's Hospital of Pennsylvania", "Hospital of the University of Pennsylvania", "St. Christopher's", "Shriners"]),
                        patient_hospital_department = random.choice(["Pediatrics","Oncology","General"]),
                        patient_treatment_description = fake.paragraph(),
                        patient_diagnosis = fake.sentence(),
                        patient_first_appt_date = fake.future_date(),
                        patient_check_in = fake.date_between_dates(date_start=patient_first_appt_date),
                        patient_check_out = fake.date_between_dates(date_start=patient_check_in),
                        patient_treating_doctor = db.Column(db.String(100)),
